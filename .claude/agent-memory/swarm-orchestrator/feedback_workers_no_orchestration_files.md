...
**Override exception — Phaser DryWash pod (committed 24ba4f4 on 2026-04-26):** the four agent .md files (`phaser-traversal-system.md`, `phaser-dry-wash-scene.md`, `phaser-bridge-construction.md`, `phaser-bridge-quest-glue.md`) and the four sequencing.yaml entries under the "Phaser DryWash" pod are USER-AUTHORED and must NOT be cleaned up. The user explicitly overrode the rule for this pod after evaluating the proposed decomposition. The override applies ONLY to these four files; the rule remains in force for any other unauthorized worker modification.
