...
| **Discovery registry** | ✅ | ✅ | **✅ (2.2)** | **✅ (2.2, consequential)** | choice/act: observe/test/investigate/learn + **walk into new areas** (exploration-triggered, no prompt) · discoverability: first-discovery **[J] tutorial** (not key-knowledge) · consequence: **NEW DISCOVERY** banner · payoff: persistent **[J]** registry (8 categories, save/load) **AND it MATTERS** — discovering the City Gate reveals the hidden Salt River expedition (affects progression, not collectible-only); 3-layer + matters-GUARD green. **Pending independent QA Fun confirmation.** |
