...
func _set_overlay_visibility() -> void:
	...
	if brake_highlight:
		brake_highlight.visible = objective.find("brakes") != -1 and not brake_check_started
	if tire_highlight:
		tire_highlight.visible = objective.find("air_") == 0 or objective == "final_air_checked"
	if chain_highlight:
		chain_highlight.visible = objective == "chain_checked" or objective == "final_chain_checked"
	if brake_rig:
