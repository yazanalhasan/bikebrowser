        ...
        quests.append(
            {
                ...
                "next_in_chain": str(data.get("next_in_chain", "")),
...
def registered_regions() -> dict[str, dict[str, Any]]:
    ...
    if not regions_path.exists():
        return {}
    raw = load_json(regions_path)
    if isinstance(raw, dict) and isinstance(raw.get("regions"), list):
        return {str(r.get("id")): r for r in raw["regions"]}
    if isinstance(raw, dict):
        return {str(k): v for k, v in raw.items() if isinstance(v, dict)}
    return {}


def scene_resource_to_path(scene_path: str) -> Path:
    if scene_path.startswith("res://"):
        return WORLD / scene_path[len("res://") :]
    return WORLD / scene_path


def scene_files() -> list[Path]:
    files: set[Path] = set()
    for region in registered_regions().values():
        scene_path = str(region.get("scenePath") or region.get("scene") or "")
        if scene_path:
            p = scene_resource_to_path(scene_path)
            if p.exists():
                files.add(p)
    known_gameplay = [
                ...
                producers.append(
                    {
                        ...
                        "target_region": str(props.get("target_region", "")),
                        "target_spawn": str(props.get("target_spawn", "")),
                    }
                )
        ...
        for idx, line in enumerate(text.splitlines(), 1):
            for m in call.finditer(line):
                ...
                if m.group(1) == "record_objective":
                    ...
                    if oid in {"id", "quest_id", "objective_id"}:
                        oid = "<dynamic>"
...
def build_producers() -> dict[str, Any]:
    ...
    registered_scene_set = {rel(scene_resource_to_path(str(r.get("scenePath") or r.get("scene") or ""))) for r in registered_regions().values() if (r.get("scenePath") or r.get("scene"))}
    gameplay_meta = [m for m in scene_meta if m["scene_file"] in registered_scene_set]
    self_test = {
        "scope": "registered gameplay region scenes plus known gameplay interaction scenes/scripts",
        "registered_gameplay_scenes_checked": len(gameplay_meta),
        "registered_gameplay_scene_parse_failures": [m for m in gameplay_meta if m["nodes"] == 0],
        "registered_gameplay_scenes_with_zero_matched_properties": [m["scene_file"] for m in gameplay_meta if m["matched_properties"] == 0],
        "note": "Zero quest properties in boot/stub regions are reported but do not block; prototype/UI/addon scenes are outside the blocking self-test.",
    }
    out = {"producers": records, "scene_parse": scene_meta, "producer_classes": producer_classes(scripts), "self_test": self_test}
...
def reachability() -> dict[str, Any]:
    ...
    scene_by_id = {rid: str(r.get("scenePath") or r.get("scene") or "") for rid, r in regions.items()}
    ...
    for scene in scene_files():
        ...
        source = next((rid for rid, sp in scene_by_id.items() if sp and scene_resource_to_path(sp).resolve() == scene.resolve()), scene.stem)
    ...
    out = {
        ...
        "states": {
            "fresh_save": walk(set()),
            "after_safety_pre_ride": walk({"act1_pre_ride_check", "first_safety_check"}),
            "after_chain_repair_complete": walk({"chain_repair"}),
            "after_bridge_review": walk({"chain_repair", "bridge_quest_1", "bridge_quest_2"}),
...
def build_matrix(truth: dict[str, Any], graded: dict[str, Any], reach: dict[str, Any]) -> dict[str, Any]:
    ...
    for q in truth["quests"]:
        ...
        for oid in q["objectives"]:
            ...
            scene_ps = [p for p in ps if p.get("scene_file")]
            reachable_ps = [p for p in scene_ps if p.get("scene_file") in scenes_any]
            qualities.extend(p["producer_quality"] for p in ps)
            if not ps:
                status = "NO_PRODUCER"
            elif scene_ps and not reachable_ps and all(p["producer_quality"] != "SCRIPT_CALL_ONLY" for p in ps):
                status = "UNREACHABLE_SCENE"
