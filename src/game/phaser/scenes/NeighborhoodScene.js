...
export default class NeighborhoodScene extends Phaser.Scene {
  ...
  createFeedbackHud() {
    ...
    this.registry.events.on('plane:built', () => {
      ...
      this.showFeedback({ message: 'It flies — Chapter 6 underway.' });
    });
    ...
    this.registry.events.on('crash:built', () => {
      this._markChapterComplete(4);
      this.showFeedback({ message: 'Crash-safe car built — Chapter 4 underway.' });
    });
    // Building the motorcycle engine completes Chapter 3's first slice.
    this.registry.events.on('engine:built', () => {
      this._markChapterComplete(3);
      this.showFeedback({ message: 'Engine tuned and built — Chapter 3 underway.' });
    });
