...
// IPC handler modules (extracted from this file) + shared context they read from.
const ipcCtx = require('./ipc/ctx');
const placesIpc = require('./ipc/places');
const marketIpc = require('./ipc/market');
const shoppingIpc = require('./ipc/shopping');
...
ipcCtx.withIpcCache = withIpcCache;

// Register the extracted IPC handler modules. Safe to do at load time — the
// handlers read ctx.searchPipeline lazily, so pipeline init can happen later.
placesIpc.register();
marketIpc.register();
shoppingIpc.register();
aiIpc.register();
...
function initializeSearchPipeline() {
  ...
  ipcCtx.fallbackHandler = fallbackHandler;
  ...
  try {
    ...
    ipcCtx.historyStore = historyStore;
  ...
  try {
    ...
    // Publish to the shared ipc context so extracted handler modules see them.
    ipcCtx.searchPipeline = searchPipeline;
    ipcCtx.imageIntentService = imageIntentService;
    ipcCtx.voiceIntentService = voiceIntentService;
...
// places:*, market:*, shopping:* handlers live in ./ipc/{places,market,shopping}.js
