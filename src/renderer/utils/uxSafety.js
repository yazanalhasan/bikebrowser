...
export function enforceUXRules(state) {
  ...
  if (UX_RULES.MUST_HAVE_HOME_BUTTON && !state.nav.homeVisible) {
  ...
  if (UX_RULES.MUST_HAVE_HOME_BUTTON && state.nav.homeVisible && !state.nav.homeIsClickable) {
  ...
  if (UX_RULES.MUST_HAVE_HEADER && !state.layout.hasHeader) {
  ...
  if (UX_RULES.MUST_HAVE_NAV_CONTAINER && !state.layout.hasNavContainer) {
  ...
  if (!state.content.hasMainContent) {
  ...
  if (UX_RULES.NO_EMPTY_SCREEN && state.content.isEmpty) {
