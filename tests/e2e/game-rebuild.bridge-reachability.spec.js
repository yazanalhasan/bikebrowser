...
test.describe('Player reachability — bridge design', () => {
  test('a real player designs the bridge (keyboard); a weak design fails, a sound one holds', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      runtime.inventorySystem.addMany(['steel', 'mesquite', 'copper_brace', 'weak_scrap']);
      ['mesquite', 'steel', 'copper_brace', 'weak_scrap'].forEach((id) => runtime.testMaterial(id));
    });
    ...
    // Flawed design: weak scrap support -> it must fail visibly.
  ...
  test('Escape exits the bridge design overlay — never trapped', async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.__GAME__.resetAct1();
      const runtime = window.__bikebrowserRebuildGame.registry.get('act1Runtime');
      runtime.inventorySystem.addMany(['steel', 'mesquite']);
      ['mesquite', 'steel'].forEach((id) => runtime.testMaterial(id));
